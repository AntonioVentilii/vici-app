...
| `src/vxp/constants.ts` + `src/vxp/awards.ts` | `src/lib/types/vxp-award.ts`, `src/lib/schema/vxp-award.schema.ts`, `src/lib/constants/vxp-economy.constants.ts`, `vxp-onboarding.constants.ts`, `achievements.constants.ts`, `referral.constants.ts`, `tokens/tokens.ic.constants.ts`, `src/lib/types/tournament.ts`, `src/satellite/services/vxp-onboarding.services.ts` | award-type set and every award amount in base units (mostly static extraction)  |
