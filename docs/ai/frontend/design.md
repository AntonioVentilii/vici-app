...
| Market card        | [`src/lib/components/market/MarketCard.svelte`](../../../src/lib/components/market/MarketCard.svelte) | ✅ Done | Compact editorial discovery card with badges, mono time-remaining, probability blocks, and challenge slot preserved.                                                                                                                                   |
...
| `style(ui): align primitives and app chrome`                          | Retunes Card, Button, Badge, Tabs, SectionHeader, StatCard, Header, and MobileNav around the app's shared card, chip, pill, and translucent chrome language; the visible fourth nav item is Social while `/leaderboard` remains the route.                                                                                                                                                                                           | §4 Components                                |
| `style(auth): align public onboarding surfaces`                       | Aligns `/signin`, `/signup`, and `/welcome` around the compact sign-in shell, visual Apple/email placeholders, four-step onboarding, starter pack, product mockup, card stack, and landing rhythm while preserving live auth providers and onboarding handoff.                                                                                                                                                                       | §5 Onboarding, §6 Landing, §8 SignInScreen   |
| `style(market): align flow and question detail`                       | Aligns Flow cards, Flow back face, market detail header, forecast/action modules, stats, and info panels around the question-detail anatomy while preserving swipe timing, trade execution, order-book polling, and sizing math.                                                                                                                                                                                                     | §4 Flow card, §5 Flow Mode, §5 Market detail |
| `style(social): align retention surfaces`                             | Reframes Social, Profile, and Portfolio as a connected retention loop: Social tabs and top-entry cards, performance-identity profile, recent activity blocks, achievements, and supporting portfolio metrics while preserving existing data services.                                                                                                                                                                                | §5 Profile, §5 Social, §5 Wallet             |
| `style(app): polish supporting surfaces`                              | Polishes market discovery, wallet, settings, notifications, empty/loading states, and supporting widgets with shared card/chrome language while preserving routing, wallet actions, preferences, and inbox behaviours.                                                                                                                                                                                                               | §5 Markets list, §5 Wallet, §4 UI primitives |
