...
| Mobile appbar      | [`src/lib/components/layout/MobileAppBar.svelte`](../../../src/lib/components/layout/MobileAppBar.svelte) | ✅ Done | Per-screen mobile top chrome (`title` or rich `titleChildren`, optional `back`, optional `right` snippet). Hidden on `md+`. Used on Markets, Market Detail, Profile, Portfolio, Arena.                                                                 |
