...
- **Use the project's tokens.** [`src/app.css`](../../../src/app.css)
  exposes `primary`, `background`, `card`, `card-border`, etc. Class-based
  utilities reference them (`bg-card`, `text-card-foreground`, etc.).
- **No raw hex** (`bg-[#0f0]`), no inline `style="color:…"`.
