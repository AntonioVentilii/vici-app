...
5. **Add a regression test for the no-collision guard** — assert that
   leaving Flow mid-beat clears `flowBeatActiveStore` so the next
   achievement reveal is not suppressed (covers `FlowMode.svelte:253`).
...
- [ ] Leaving Flow mid-beat clears `flowBeatActiveStore`; a subsequently
      crossed Menagerie tier still reveals (regression test green).
...
Resolve before building (workflow step 4); record the choice back into
[Scope](#scope).

- **Call-13 overtime character — `oracle` (design source) vs `trickster`
  (shipped).** Which voice owns the rare overtime push at call 13?
- **Volume ladder confirmation.** Is `10/100/500/1000 → +50/+100/+250/+500`
  (with `1` and `25` moment-only, no VXP) the final economy, or does the
  "×50 from 50 up" framing in the source's comment imply a different
  ladder?
- **Any further changes to the motion/reward system** beyond the two
  reconciliation items above — to be enumerated here once Giovanni
  confirms the specific tweaks.

## Decisions

<!-- Append the resolved open decisions here as they are settled, with the why. -->
