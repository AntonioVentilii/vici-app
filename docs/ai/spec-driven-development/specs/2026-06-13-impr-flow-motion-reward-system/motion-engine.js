...
(function () {
  ...
  const POOLS = {
    'first-call':   ["You're in!", "First call's on the board.", "And you're off!", "Welcome to the game."],
    ...
    'welcome-back': ['Welcome back.', 'Look who’s back.', 'There you are.', 'Right on time.', 'Missed you.'],
    'comeback':     ["Knew you’d be back.", 'The record was waiting.', 'No grudges — let’s go.', 'Right where you left off.'],
  ...
  const VOLUME = {
    1:    { bonus: 0,   character: 'vici',   copy: 'first-call', sub: 'FIRST CALL', duration: 1900, haptic: [12, 30, 12, 30, 12] },
    10:   { bonus: 50,  character: 'oracle', copy: 'Ten calls in.',      sub: '10 LIFETIME',  duration: 1300, haptic: [25, 30, 25] },
    25:   { bonus: 0,   character: 'oracle', copy: 'A regular now.',     sub: '25 LIFETIME',  duration: 1200, haptic: 12 },
    100:  { bonus: 100, character: 'oracle', copy: 'A hundred. Respect.',sub: '100 LIFETIME', duration: 1700, haptic: [12, 30, 12, 30, 12, 30, 60], badge: 'CENTURION' },
    500:  { bonus: 250, character: 'oracle', copy: 'Five hundred.',      sub: '500 LIFETIME', duration: 1500, haptic: [25, 30, 25] },
    1000: { bonus: 500, character: 'oracle', copy: 'Legend.',           sub: '1,000 LIFETIME', duration: 2000, haptic: [40, 30, 40, 30, 80], title: 'vici' },
  };
  ...
  const OT_RHYTHM = {
    11: { character: 'trickster', pool: 'ot-enter', sub: 'OVERTIME', duration: 1300, haptic: [10, 40, 10] },
  ...
  function recordSwipe(input) {
    const {
      side, isContrarian,
      dailyDone, dailyTarget, dailyJustCompleted, dayStreak,
      firstLeaderboard,
      wildcards = true, isComeback = false,
    } = input || {};
    const state = load();
    ...
    if (isComeback && dailyDone === 1) {
      beat = { kind: 'comeback', character: 'vici', copy: pick(state, 'comeback'), sub: 'COMEBACK', duration: 1700, haptic: [12, 30, 12, 30, 12] };
    }
    ...
    if (dailyJustCompleted) {
      ...
      if (overtime) {
        ...
        if (!beat) beat = { kind: 'overtime-complete', character: 'flame', tier: tier === 'NONE' ? 'SPARK' : tier, copy: pick(state, 'overtime-15'), sub: 'OVERTIME COMPLETE', bonus: 25, duration: 1900, haptic: [25, 30, 25, 40, 60] };
      } else {
        // regular daily ten — celebration + streak, but NO VXP (deflation-safe)
        if (!beat) beat = { kind: 'daily-complete', character: 'flame', tier: tier === 'NONE' ? 'SPARK' : tier, copy: isComeback ? pick(state, 'streak-reborn') : pick(state, 'daily-ten'), sub: isComeback ? 'STREAK RESTARTED' : 'STREAK +1', duration: 1900, haptic: [25, 30, 25, 40, 60] };
      }
    }
    ...
    if (VOLUME[calls]) {
      ...
      if (!beat) {
        beat = {
          kind: 'volume-' + calls, character: m.character,
          copy: POOLS[m.copy] ? pick(state, m.copy) : m.copy,
          sub: m.sub, bonus: m.bonus || 0, duration: m.duration, haptic: m.haptic,
          badge: m.badge, title: m.title, count: calls,
        };
      }
    }
    ...
    if (!beat && overtime && OT_RHYTHM[dailyDone]) {
      ...
      beat = { kind: 'ot-' + dailyDone, character: m.character, copy: pick(state, m.pool), sub: m.sub, duration: m.duration, haptic: m.haptic };
    }
    ...
    if (!beat && calls > 1) {
      if (side === 'YES' && !state.flags.firstYes) {
        ...
        beat = { kind: 'first-yes', character: 'vici', copy: pick(state, 'first-yes'), duration: 900, haptic: [12, 30, 12] };
      } else if (side === 'NO' && !state.flags.firstNo) {
        state.flags.firstNo = true;
        beat = { kind: 'first-no', character: 'vici', copy: pick(state, 'first-no'), duration: 900, haptic: [12, 30, 12] };
      } else if (isContrarian && !state.flags.firstContrarian) {
        state.flags.firstContrarian = true;
        beat = { kind: 'first-contrarian', character: 'trickster', copy: pick(state, 'first-contra'), sub: 'CONTRARIAN', badge: 'CONTRARIAN', duration: 1100, haptic: [10, 40, 10, 40, 10] };
      } else if (firstLeaderboard && !state.flags.firstLeaderboard) {
        state.flags.firstLeaderboard = true;
        beat = { kind: 'first-leaderboard', character: 'oracle', copy: 'On the board.', duration: 1200, haptic: [40, 60, 40] };
      }
    }
    ...
    if (!beat && !overtime && state.day.sched[dailyDone] && dailyDone < target) {
      ...
      const m = RHYTHM[key];
      beat = { kind: 'daily-' + dailyDone, character: m.character, copy: pick(state, m.pool), duration: m.duration, haptic: m.haptic };
    }
    ...
    if (!beat && wildcards && calls > 1 && Math.random() < 1 / 6) {
      ...
      beat = { kind: 'wildcard', character: 'trickster', copy: pick(state, 'wildcard'), treat: treat, sub: 'SURPRISE', duration: 1250, haptic: [10, 40, 10] };
    }
