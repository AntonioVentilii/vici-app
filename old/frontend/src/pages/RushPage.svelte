<script lang="ts">
	...
	$: rushMarkets = $markets.markets.filter((m) => m.status === 'open').slice(0, 10);
	...
	const executeTrade = async (positionType: 'yes' | 'no') => {
		...
		try {
			...
			await actor.placePosition(currentMarket.id, positionType as any, BigInt(amount));

			toast.success(`Trade placed: ${positionType.toUpperCase()} for Ꝟ ${amount}`);
			advanceToNext();
			userProfile.refresh();
		} catch (error: any) {
...
<div class="container mx-auto px-4 py-8">
	...
	<div class="mb-6 flex items-center justify-between">
		<div class="flex items-center space-x-4">
			<h1 class="text-foreground text-2xl font-bold">Rush Mode</h1>
	...
	{:else if rushMarkets.length === 0}
		<div class="flex min-h-[60vh] flex-col items-center justify-center space-y-6">
			<div class="space-y-2 text-center">
				<h2 class="text-foreground text-2xl font-bold">No Markets Available</h2>
				<p class="text-muted-foreground max-w-md">
					There are no eligible open markets at the moment. Check back later or browse all markets.
				</p>
			</div>
			<Button on:click={() => onNavigate('markets')} size="lg">Browse Markets</Button>
		</div>
	{:else if currentMarket}
		<!-- Instructions -->
		<div class="mb-6 rounded-lg border border-slate-100 bg-slate-50 p-4">
			<p class="text-muted-foreground text-center text-sm">
				<span class="font-semibold">Swipe or use keyboard:</span> Right/→ = YES • Left/← = NO • Up/↑ =
				Skip
			</p>
		</div>
		...
		>
				...
				<div class="space-y-4 p-6">
					<div class="flex items-start justify-between gap-4">
						...
						{#if currentMarket.inviteOnly}
							<Badge variant="outline" class="shrink-0">Invite Only</Badge>
						{/if}
					</div>
					...
					<div class="grid grid-cols-2 gap-4">
						<div class="rounded-lg border border-green-100 bg-green-50 p-4">
							<div class="text-muted-foreground mb-1 text-xs">YES Odds</div>
							<div class="text-2xl font-bold text-green-600">
								{(currentMarket.oddsYes * 100).toFixed(0)}%
							</div>
						</div>
						<div class="rounded-lg border border-red-100 bg-red-50 p-4">
							<div class="text-muted-foreground mb-1 text-xs">NO Odds</div>
					...
					<div class="space-y-2">
						<label for="trade-amount" class="text-sm font-medium">Trade Amount (Ꝟ)</label>
						<Input
							id="trade-amount"
							type="number"
			...
			<div class="grid grid-cols-3 gap-4">
				<Button
					variant="destructive"
					class="flex h-20 flex-col items-center justify-center"
					on:click={handleNo}
					disabled={isProcessing}
				>
					{#if isProcessing}
				...
				<Button
					variant="outline"
					class="flex h-20 flex-col items-center justify-center border-2"
					on:click={handleSkip}
					disabled={isProcessing}
				>
				...
				<Button
					class="flex h-20 flex-col items-center justify-center bg-green-600 text-white hover:bg-green-700"
					on:click={handleYes}
					disabled={isProcessing}
				>
					{#if isProcessing}
