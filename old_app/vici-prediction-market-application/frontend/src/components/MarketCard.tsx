...
export default function MarketCard({ market, onViewMarket }: MarketCardProps) {
  const getStatusBadge = (status: MarketStatus) => {
    switch (status) {
      case MarketStatus.open:
        return <Badge variant="default" className="bg-chart-2">Open</Badge>;
  ...
  return (
    <Card className="hover:shadow-lg transition-shadow cursor-pointer group" onClick={() => onViewMarket(market.id)}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-bold text-chart-2">{Math.round(market.oddsYes * 100)}¢</div>
          </div>
          <div className="space-y-1">
            <div className="text-xs text-muted-foreground">No</div>
            <div className="text-2xl font-bold text-destructive">{Math.round(market.oddsNo * 100)}¢</div>
          </div>
        </div>
