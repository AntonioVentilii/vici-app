...
import { useGetMarketComments, useCreateComment, useModerateComment, useIsCallerAdmin } from '../hooks/useQueries';
  ...
  return (
    <div className="space-y-4">
      ...
      {isAuthenticated ? (
        ...
        <div className="p-6 bg-accent/20 rounded-lg border border-border text-center space-y-3">
          <LogIn className="h-10 w-10 mx-auto text-muted-foreground" />
          <div>
            <p className="text-sm font-medium text-foreground mb-1">Login to Join the Discussion</p>
            <p className="text-xs text-muted-foreground mb-4">Sign in to share your thoughts and insights</p>
          </div>
                ...
                >
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <p className={`text-sm ${comment.isHidden ? 'text-muted-foreground italic' : ''}`}>
                        {comment.isHidden && !isAdmin ? '[Comment hidden by moderator]' : comment.content}
                      </p>
                    </div>
                  </div>
                </div>
