...
import type { MarketSnapshot, Position, Transaction, UserProfile, PositionType, UserRole, MarketDepthPosition, AnalyticsDataPoint, Comment, WalletBalance } from '../backend';
...
export function usePlaceTrade() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['marketAnalytics', variables.marketId.toString()] });
