...
import { useGetMarket, usePlaceTrade, useGetCallerUserProfile, useGetMarketDepth, useGetMarketAnalytics, useGetMarketComments, useCreateComment, useModerateComment, useIsCallerAdmin, useGetInvitedUsers, useInviteUsers, useGetFriendsList, useGetUserProfile } from '../hooks/useQueries';
...
import { ArrowLeft, TrendingUp, TrendingDown, Clock, Info, Users, MessageSquare, BarChart2, Lock, UserPlus, X, LogIn } from 'lucide-react';
...
export default function MarketDetailPage({ marketId, onBack }: MarketDetailPageProps) {
  ...
  const isInvited = market?.inviteOnly ? invitedUsers.some(p => p.toString() === callerPrincipal?.toString()) : true;
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <h1 className="text-3xl font-bold text-foreground mb-2">{market.title}</h1>
                ...
                {market.status === MarketStatus.resolved && market.resolvedOutcome !== undefined && (
                  <div className="p-4 bg-accent rounded-lg">
                    <p className="text-sm font-medium text-foreground">
                      Resolved: <span className="text-lg font-bold">{market.resolvedOutcome ? 'YES' : 'NO'}</span>
                    </p>
                  </div>
                )}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-6 bg-chart-2/10 border-2 border-chart-2 rounded-lg text-center">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    <TrendingUp className="h-5 w-5 text-chart-2" />
                    <span className="text-sm font-medium text-muted-foreground">YES</span>
                  </div>
                  <p className="text-3xl font-bold text-chart-2">{(market.oddsYes * 100).toFixed(1)}%</p>
                </div>
                <div className="p-6 bg-destructive/10 border-2 border-destructive rounded-lg text-center">
                  ...
                  <p className="text-3xl font-bold text-destructive">{(market.oddsNo * 100).toFixed(1)}%</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              {!isAuthenticated ? (
                <div className="text-center py-8 space-y-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground mb-1">Login to Trade</p>
                    <p className="text-xs text-muted-foreground mb-4">Sign in to place your bets on this market</p>
                  </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm font-medium text-foreground mb-1">Invite-only Market</p>
                  <p className="text-xs text-muted-foreground">You are not invited to this market</p>
                </div>
              ) : isClosed ? (
                <div className="text-center py-8">
                  <Info className="h-12 w-12 mx-auto mb-3 text-muted-foreground" />
                  <p className="text-sm text-muted-foreground">This market is closed for trading</p>
                </div>
              ) : (
                <>
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        ...
                        className={selectedPosition === 'yes' ? 'bg-chart-2 hover:bg-chart-2/90' : ''}
                      >
                      ...
                      <Button
                        ...
                        className={selectedPosition === 'no' ? 'bg-destructive hover:bg-destructive/90' : ''}
                      >
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <Select value={selectedFriend} onValueChange={setSelectedFriend}>
                              ...
                              <SelectContent>
                                {friendsList
                                  .filter(friend => !invitedUsers.some(u => u.toString() === friend.toString()))
                                  .map((friend) => {
                                    const FriendOption = () => {
                                      const { data: profile } = useGetUserProfile(friend);
                                      return (
                                        <SelectItem key={friend.toString()} value={friend.toString()}>
                                          {profile?.nickname || friend.toString().slice(0, 12) + '...'}
                                        </SelectItem>
                                      );
                                    };
