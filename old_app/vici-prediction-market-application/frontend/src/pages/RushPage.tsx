...
import { PositionType, MarketStatus } from '../backend';
...
export default function RushPage({ onNavigate }: RushPageProps) {
  ...
  const executeTrade = async (positionType: PositionType) => {
    ...
    try {
      ...
      toast.success(`Trade placed: ${positionType === PositionType.yes ? 'YES' : 'NO'} for Ꝟ ${amount}`);
      advanceToNext();
    } catch (error: any) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6">
          <div className="text-center space-y-2">
            <h2 className="text-2xl font-bold text-foreground">No Markets Available</h2>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          <h1 className="text-2xl font-bold text-foreground">Rush Mode</h1>
      ...
      <div className="mb-6 p-4 bg-accent/50 rounded-lg">
        <p className="text-sm text-muted-foreground text-center">
          <span className="font-semibold">Swipe or use keyboard:</span> Right/→ = YES • Left/← = NO • Up/↑ = Skip
        </p>
      </div>
