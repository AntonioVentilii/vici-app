...
if ! echo "$LIST_OUT" | grep -q "engine_id = \"$EXPECTED_ENGINE_ID\""; then
...
if ! echo "$LIST_OUT" | grep -q "name = \"$ENGINE_NAME\""; then
  fail "engine name $ENGINE_NAME not found. list_engines output:
$LIST_OUT"
fi
pass "engine name is $ENGINE_NAME"

# 2. Admins include satellite principal.
if ! echo "$LIST_OUT" | grep -q "principal \"$SATELLITE_PRINCIPAL\""; then
...
# 3. Allowed roles include Creator and OracleAdmin.
# Engines are laid out roughly as:
#   record { engine_id = "..."; name = "..."; ... allowed_roles = vec { variant { Creator }; variant { OracleAdmin } }; ... }
if ! echo "$LIST_OUT" | grep -qE 'allowed_roles = vec \{[^}]*variant \{ Creator \}'; then
  fail "allowed_roles does not contain variant { Creator }"
fi
pass "allowed_roles contains Creator"

if ! echo "$LIST_OUT" | grep -qE 'allowed_roles = vec \{[^}]*variant \{ OracleAdmin \}'; then
  fail "allowed_roles does not contain variant { OracleAdmin }"
fi
