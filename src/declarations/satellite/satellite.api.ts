...
const AppGetCurrentTournamentResultSchema = j.strictObject({tournament: j.optional(j.strictObject({id: j.string(), month_start_ms: j.number(), month_end_ms: j.number(), bracket_size: j.number(), state: j.enum(['in_flight', 'concluded']), seeded_league_ids: j.array(j.string()), created_at_ms: j.number()})), matches: j.array(j.strictObject({tournament_id: j.string(), round: j.enum(['r1', 'quarter', 'semifinal', 'final']), index: j.number(), from_league_id: j.optional(j.string()), to_league_id: j.optional(j.string()), from_acc: j.optional(j.number()), to_acc: j.optional(j.number()), winner_league_id: j.optional(j.string()), start_ms: j.number(), end_ms: j.number()}))});
...
const AppDeleteMyAccountResultSchema = j.strictObject({ok: j.boolean(), reason: j.optional(j.enum(['owns_non_empty_league', 'invalid_input'])), blockingLeagueIds: j.optional(j.array(j.string())), docsDeleted: j.optional(j.number())});
