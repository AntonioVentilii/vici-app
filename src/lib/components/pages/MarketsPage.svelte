<script lang="ts">
	...
	// World-Cup release gating: WC markets surface only once their on-chain
	// trading start (`start_ns`) has arrived; WC markets with no start are live.
	// Applied to the discovery feeds (available + trending) below; non-WC markets
	// pass through, and the viewer's explicit Saved watchlist is intentionally
	// left ungated. Re-derives on the minute tick so a market appears the moment
	// its release lands.
