	...
	([$requests, $profiles, $user, $locale]) => {
		...
		return $requests.map((doc: Doc<Relation>) => {
			...
			return {
				...
				// into view, so a tap lands on the Accept affordance instead of
				// a bare tab the recipient must hunt through. The relation id
				// contains `#`, so it is percent-encoded into the query value.
