 ...
 *
 * Used to boost the markets tied to the team the user picked during
 * onboarding (`preferences.favoriteParticipantId`) in the Flow ranker —
 * see `rankMarkets`. Returns an empty set when no participant is selected
 * (the empty-string skip sentinel) or the id isn't part of the event, so
 * callers can pass the raw preference value without guarding first.
 */
