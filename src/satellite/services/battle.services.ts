...
export const assertSetBattle = ({
	...
	if (isNullish(current)) {
		...
		if (hasScoreFields) {
			throw new Error('New battles must not carry scoreA / scoreB / winner.');
		}
	...
	if (transition === 'proposed->accepted') {
		...
		if (hasScoreFields) {
			throw new Error('battles accept must not carry scoreA / scoreB / winner.');
		}
		...
		if (hasScoreFields) {
			throw new Error('battles kickoff must not carry scoreA / scoreB / winner.');
		}
		...
		if (hasScoreFields) {
			throw new Error('battles pre-resolved must not carry scoreA / scoreB / winner.');
		}
	}
};
